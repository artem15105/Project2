        ...
        if (vehicleMode == VehicleMode.Player && GameUI.manage.gameStarted)
        {
            if (AIControl.manage.controlMode == ControlMode.Simple)
            {
                ...
                if (carWheels.wheels.frontWheelDrive || carWheels.wheels.backWheelDrive)
                {
                    ...
                    if (Input.GetKey("w") || ControlUi.top)
                    {
                        accel = 1;
                    }
                    if (Input.GetKey("s") || ControlUi.back)
                    {
                        accel = -1;
                    }

                    if (Input.GetKey("a") || ControlUi.left)
                   {
                        if(steer >= -1)
                        {
                            steer -= 0.30f;
                   ...
                   else if (Input.GetKey("d") || ControlUi.right)
                   {
                       if (steer <= 1)
                       {
                            steer += 0.30f;
                    ...
                    //accel = Input.GetAxis("Vertical"); noActual
